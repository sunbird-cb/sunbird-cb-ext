...
public interface StaffRepository extends CassandraRepository<StaffInfoModel, StaffInfoPrimaryKeyModel> {
    ...
    public List<StaffInfoModel> updateStaffDetails(int totalPositionFilled, int totalPositionVacant, String rootOrg, String id);
	...
	@Query("select * from org_staff_position where orgId=?0 and position=?1 ALLOW FILTERING")
