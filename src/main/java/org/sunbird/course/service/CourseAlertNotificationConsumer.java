	...
	public void processCourseReminderMessage(ConsumerRecord<String, String> data) {
		...
		try {
			if (StringUtils.isNoneBlank(value)) {
				if (value.equalsIgnoreCase(serverProperties.getIncompleteCourseAlertMessageKey())) {
					CompletableFuture.runAsync(() -> {
						incompleteCourseService.initiateCourseReminderEmail();
					});
				} else if (value.equalsIgnoreCase(serverProperties.getLatestCourseAlertMessageKey())) {
					CompletableFuture.runAsync(() -> {
