...
import org.sunbird.course.model.Filters;
import org.sunbird.course.model.LastUpdatedOn;
import org.sunbird.course.model.NewCourseData;
import org.sunbird.course.model.Request;
import org.sunbird.course.model.RequestData;
import org.sunbird.course.model.SortBy;
		...
		try {
			if (!coursesDataMapList.isEmpty()) {
				...
				for (int i = 0; i < coursesDataMapList.size(); i++) {
					...
					params.put(Constants.COURSE_KEYWORD + j + Constants.THUMBNAIL,
							coursesDataMapList.get(i).getThumbnail());
					params.put(Constants.COURSE_KEYWORD + j + Constants._NAME,
							coursesDataMapList.get(i).getCourseName());
					params.put(Constants.COURSE_KEYWORD + j + Constants._DURATION,
							coursesDataMapList.get(i).getDuration());
					params.put(Constants.COURSE_KEYWORD + j + Constants._DESCRIPTION,
							coursesDataMapList.get(i).getDescription());
				}
...
//                mailList.addAll(Arrays.asList(extraEmails.split(",", -1)));
				...
				for (int i = 0; i < mailList.size(); i += chunkSize) {
					...
					new NotificationUtil().sendNotification(
							Arrays.asList("nitin.raj@tarento.com", "juhi.agarwal@tarento.com"), params,
							"support@igot-dev.in",
							...
							Constants.NEW_COURSES, Constants.NEW_COURSES_MAIL_SUBJECT);
	...
	public NewCourseData getLatestAddedCourses() {
		try {
			...
			if (!lastUpdatedOn.getMax().equalsIgnoreCase(lastUpdatedOn.getMin())) {
				...
				request.setOffset(0);
				request.setLimit(
						Integer.parseInt(PropertiesCache.getInstance().getProperty(Constants.NEW_COURSES_EMAIL_LIMIT)));
				...
				String url = PropertiesCache.getInstance().getProperty(Constants.KM_BASE_HOST)
						+ PropertiesCache.getInstance().getProperty(Constants.CONTENT_SEARCH);
	...
	private List<CoursesDataMap> setCourseMap(NewCourseData newCourseData) {
		...
		for (Content course : coursesList) {
			try {
				...
				if (!StringUtils.isEmpty(course.getIdentifier()) && !StringUtils.isEmpty(course.getName())
						&& !StringUtils.isEmpty(course.getPosterImage())
						&& !StringUtils.isEmpty(course.getDuration())) {
					...
					coursesDataMap
							.setCourseUrl(PropertiesCache.getInstance().getProperty(Constants.COURSE_URL) + courseId);
	...
	private String calculateMinValue(LocalDate maxValue) {
		...
		if (!emailRecords.isEmpty()) {
			minValue = !StringUtils.isEmpty(emailRecords.get(0).get(Constants.LAST_SENT_DATE))
					? (String) emailRecords.get(0).get(Constants.LAST_SENT_DATE)
					: "";
		}
		if (StringUtils.isEmpty(minValue)) {
			minValue = maxValue
					.minusDays(Long.valueOf(
							PropertiesCache.getInstance().getProperty(Constants.NEW_COURSES_SCHEDULER_TIME_GAP)) / 24)
					.toString();
		}
		return minValue;
	}
		...
		for (Map<String, Object> userDetail : userDetails) {
			try {
				if (userDetail.get(Constants.PROFILE_DETAILS_KEY) != null) {
					Map profileDetails = new ObjectMapper()
							.readValue((String) userDetail.get(Constants.PROFILE_DETAILS_KEY), Map.class);
					if (!profileDetails.isEmpty()) {
						Map<String, Object> personalDetailsMap = (Map<String, Object>) profileDetails
								.get(Constants.PERSONAL_DETAILS);
						if (!personalDetailsMap.isEmpty() && personalDetailsMap.get(Constants.PRIMARY_EMAIL) != null
								&& !excludeEmailList.contains(personalDetailsMap.get(Constants.PRIMARY_EMAIL))) {
