...
public class AssessmentUtilServiceV2Impl implements AssessmentUtilServiceV2 {
    ...
    @Override
    public Map<String, Object> generateQumlAssessmentScores(List<Map<String, Object>> userQuestionList) {
        try {
            Integer correct = 0;
            Integer blank = 0;
            Integer inCorrect = 0;
            ...
            List<Object> originalQuestionListObject = userQuestionList.stream()
                    .flatMap(x -> desiredKeys.stream()
                            .filter(x::containsKey)
                            .map(x::get)
                    ).collect(toList());
            List<String> originalQuestionList = originalQuestionListObject.stream()
